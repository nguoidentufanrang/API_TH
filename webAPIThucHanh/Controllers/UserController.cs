...
namespace webAPIThucHanh.Controllers
{
	...
	public class UserController : ControllerBase
	{
		...
		[HttpPost]
		[Route("Login")]
		public async Task<IActionResult> Login([FromBody] LoginRequetsDTO loginRequest)
		// khai báo model cho Login 
		{
			var user = await _userManager.FindByEmailAsync(loginRequest.Username);
			if (user != null)
			{
				var checkPasswordResult = await _userManager.CheckPasswordAsync(user, loginRequest.Password);// kiểm tra password nhập vào khớp với password lưu ở database
